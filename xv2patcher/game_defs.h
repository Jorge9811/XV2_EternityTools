...
struct Battle_Mob
{
	...
	uint8_t unk_4B0[0x1D14-0x4B0]; 
	int32_t loaded_var; // 1D14 if >= 0, char is loaded
	uint8_t unk_1D18[0x1E1C-0x1D18];
	int32_t trans_control; // 1E1C
	// ...
...
CHECK_FIELD_OFFSET(Battle_Mob, loaded_var, 0x1D14);
CHECK_FIELD_OFFSET(Battle_Mob, trans_control, 0x1E1C);
...
{
	uint8_t unk_00[0x3F0]; 
	Battle_HudCharInfo char_infos[MAX_MOBS];
	// ...
};
CHECK_FIELD_OFFSET(Battle_HudCockpit, char_infos, 0x3F0);
