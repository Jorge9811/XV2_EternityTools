...
// Remainder of remainder: addresses are 3 bytes more than the instruction address
#define REMAINDER_13 0x2E48C5
#define REMAINDER_14 0x2E4C35

// Address of stagegt within XG::Game::Battle::Core::MainSystem 
#define STAGEGT_ADDRESS	0x61D0
#define MAINSYSTEM_SIZE	0xACD0
...
PUBLIC void PatchStageDef1Off8_Remainder13(void *buf)
{
	size_t rel = RelAddress(buf);
	
	if (rel != REMAINDER_13)
	{
		UPRINTF("Update the addresses of patches PatchStageDef1Off8_3, PatchStageDef1Off8_4, PatchStageNum1 and PatchStageNum2\n");
		exit(-1);
	}
...
PUBLIC void PatchStageDef1Off8_Remainder14(void *buf)
{
	size_t rel = RelAddress(buf);
...
typedef void (* SetFarClipType)(float **);
SetFarClipType SetFarClip;
...
PUBLIC void SetupSetFarClip(SetFarClipType orig)
{
	SetFarClip = orig;
}

PUBLIC void OverrideFarClip(float **obj)
{
	SetFarClip(obj);
