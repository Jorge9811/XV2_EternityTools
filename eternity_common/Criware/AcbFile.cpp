...
uint32_t AcbFile::CueIndexToCueId(uint32_t idx) const
{
    /*uint32_t id;
      ...
      if (CueTable.GetNumColumns() > 0 && CueTable.GetDword("CueId", &id, idx))
        return id;
...
bool AcbFile::AddTrack(const std::string &name, uint16_t awb_idx, bool external, uint32_t track_length_ms, uint32_t track_length_smp, bool loop_flag, uint16_t sample_rate, uint8_t num_channels, uint16_t cmd_idx1, uint16_t cmd_idx2, uint32_t *cue_id, uint32_t *cue_idx, uint32_t *track_idx)
{
    ...
    {
        if (!SynthTable.SetWord("CommandIndex", (external) ? 0 : 0xFFFF, _cue_idx, false))
            return false;
    }
...
bool AcbFile::GetTrackCommandIndex(uint32_t cue_idx, uint16_t *cmd_idx) const
{
    if (TrackTable.GetNumColumns() == 0)
        return false;

    return TrackTable.GetWord("CommandIndex", cmd_idx, cue_idx);
}

bool AcbFile::GetSequenceCommandIndex(uint32_t cue_idx, uint16_t *cmd_idx) const
{
    if (SequenceTable.GetNumColumns() == 0)
        return false;

    return SequenceTable.GetWord("CommandIndex", cmd_idx, cue_idx);
}
