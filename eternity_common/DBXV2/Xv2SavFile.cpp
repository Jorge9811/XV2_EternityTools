...
bool Xv2SavFile::Load(const uint8_t *buf, size_t size)
{
    ...
    {
        is_encrypted = true;
		is_v8 = (size == ENCRYPTED_SIZE_V8);

        if (!is_v8 && size != ENCRYPTED_SIZE)
        {
            DPRINTF("%s Encrypted size mismatch (got 0x%Ix)\n", FUNCNAME, size);
            return false;
        }
		...
		uint32_t decrypted_size = (is_v8) ? DECRYPTED_SIZE_V8 : DECRYPTED_SIZE;
    ...
    {
        is_v8 = (size == DECRYPTED_SIZE_V8);
		is_encrypted = false;
        data.resize(size);
        memcpy(data.data(), buf, size);
    }

    if (data.size() != DECRYPTED_SIZE && data.size() != DECRYPTED_SIZE_V8)
    {
...
uint8_t *Xv2SavFile::Save(size_t *psize)
{
    ...
    {
        uint32_t decrypted_size = (is_v8) ? DECRYPTED_SIZE_V8 : DECRYPTED_SIZE;
		uint32_t encrypted_size = (is_v8) ? ENCRYPTED_SIZE_V8 : ENCRYPTED_SIZE;
		
		size += 0x80 + ((is_v8) ? 0x28 : 0x30);
