...
STATIC_ASSERT_STRUCT(SEVHeader, 0x10);

typedef struct
{
    uint32_t char_id; // 0
    uint32_t costume_id; // 4
    uint32_t num_cev_entries; // 8
    uint32_t unk_0C; // zero
} PACKED SEVEntry;

STATIC_ASSERT_STRUCT(SEVEntry, 0x10);
...
STATIC_ASSERT_STRUCT(SEVCharEvents, 0x10);
...
STATIC_ASSERT_STRUCT(SEVEvent, 0xC);
...
STATIC_ASSERT_STRUCT(SEVEventEntry, 0x24);
...
struct SevEntry
{
    ...
    {
        return (char_id == rhs.char_id && costume_id == rhs.costume_id && chars_events == rhs.chars_events);
    }
