...
struct PACKED CSTEntry
{
    ...
    uint32_t dlc; // 0x14
    uint32_t is_custom_costume; // 0x18
    uint32_t cac_index; // 0x1C - Added in game v 1.10
	uint32_t var_type_after_TU9_order; // 0x20 - Added in game 1.14. Whatever this shit is, is -1 in all chars, except TU0 ant TU1 where it is 0 and 1 respectively.
};
CHECK_STRUCT_SIZE(CSTEntry, 0x24);
