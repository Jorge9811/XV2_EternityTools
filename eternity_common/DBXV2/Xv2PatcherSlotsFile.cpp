...
static std::string DlcToString(int dlc)
{
    std::string ret;
    ...
    switch (dlc)
    {
        ...
        default:
            DPRINTF("%s: Unknown dlc %d.\n", FUNCNAME, dlc);
    }

    return ret;
}

static int StringToDlc(const std::string &dlc)
{
    if (dlc == "Dlc_Def")
        return 1;
    else if (dlc == "Dlc_Gkb")
...
bool Xv2PatcherSlotsFile::Load(const uint8_t *buf, size_t size)
{
    ...
    while (i < raw_string.length())
    {
        ...
        while (raw_string[i] == '[')
        {
            ...
            int dlc = Utils::GetSigned(fields[7]);
