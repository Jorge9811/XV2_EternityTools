// half.hpp must be the first one included!
#include "half.hpp"
#include "EanFile.h"
#include "debug.h"
...
bool EanFile::ScaleAnim(uint16_t idx, float x, float y, float z)
{
    ...
    for (uint16_t i = 0; i < ahdr->nodes_count; i++)
    {
        ...
        for (uint16_t j = 0; j < node->keyframed_animation_count; j++)
        {
            ...
            for (uint16_t k = 0; k < kfa->keyframe_count; k++)
            {
                float fx = half_float::detail::half2float(c16[k].x);
                float fy = half_float::detail::half2float(c16[k].y);
                float fz = half_float::detail::half2float(c16[k].z);

                fx *= x;
                fy *= y;
                fz *= z;

                c16[k].x = half_float::detail::float2half<std::round_to_nearest>(fx);
                c16[k].y = half_float::detail::float2half<std::round_to_nearest>(fy);
                c16[k].z = half_float::detail::float2half<std::round_to_nearest>(fz);
            }
        }
    }
