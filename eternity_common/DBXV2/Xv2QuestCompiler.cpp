...
static const std::vector<InstructionDef> instructions_defs =
{
    ...
    { QED_COND_UNK60, "Unk60", false, { PARAM_BOOLEAN, PARAM_INTEGER, PARAM_INTEGER } },
...
bool Xv2QuestCompiler::MaintenanceAudio()
{
    ...
    for (size_t i = 0; i < dialogue_acb.size(); i++)
    {
        ...
        {
            std::vector<uint32_t> cue_idxs;
            std::vector<uint32_t> cue_ids;
            std::vector<std::string> names;

            acb.TrackIndexToCueIndexes(t, cue_idxs);
            cue_ids.resize(cue_idxs.size());
            names.resize(cue_ids.size());

            for (size_t c = 0; c < cue_ids.size(); c++)
            {
                cue_ids[c] = acb.CueIndexToCueId(cue_idxs[c]);
...
{
    if (reward.flags != 0 && reward.flags != 2 && reward.flags != 5 && reward.flags != 6 && reward.flags != 9)
    {
    ...
    if (reward.flags == 5 || reward.flags == 6 || reward.flags == 9)
