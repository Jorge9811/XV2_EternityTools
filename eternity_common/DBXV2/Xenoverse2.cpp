...
bool Xenoverse2::InitIdb(bool costumes, bool accesories, bool talisman, bool skills, bool material, bool battle)
{
    if (!xv2fs)
        return false;
...
uint32_t Xenoverse2::SetSound(AcbFile *acb, Afs2File *awb, const std::string &name, HcaFile &hca, bool is_external_awb)
{
    if (!acb || !awb)
        return (uint32_t)-1;
    ...
    if (!acb->AddTrack(name, awb_idx, is_external_awb, (uint32_t)(hca.GetDuration()*1000.0f), hca.GetNumSamples(),
                       false, (uint16_t)hca.GetSampleRate(), (uint8_t)hca.GetNumChannels(), 2, 6, &cue_id, nullptr, nullptr))
    {
        //DPRINTF("Fail here");
        return (uint32_t)-1;
    }
...
uint32_t Xenoverse2::SetCssSound(const std::string &name, bool english, HcaFile &hca)
{
    ...
    return SetSound(acb, awb, name, hca);
}
