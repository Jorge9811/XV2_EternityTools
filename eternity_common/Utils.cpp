...
std::string Utils::UnsignedToHexString(uint64_t value, bool zeropad, bool prefix)
{
    ...
    {
        if (prefix)
            sprintf(temp, "0x%08I64x", value);
        else
            sprintf(temp, "%08I64x", value);
    }
...
size_t Utils::GetMultipleStrings(const std::string & str_param, std::vector<std::string> & list, char separator, bool omit_empty_strings)
{
	size_t pos;
